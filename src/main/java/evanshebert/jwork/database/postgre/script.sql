...
-- INVOICE
CREATE TABLE recruiter(id SERIAL PRIMARY KEY, name text NOT NULL, email text NOT NULL, phoneNumber text NOT NULL, province text, city text, description text);
CREATE TABLE job(
...
CREATE TABLE invoice(
                        ...
                        invoiceStatus text DEFAULT 'Ongoing', paymentType text DEFAULT 'EwalletPayment', bonus_id int DEFAULT NULL, adminFee bigint DEFAULT 0,
...
FROM invoice i INNER JOIN job j ON i.job_id = j.id INNER JOIN jobseeker s ON i.jobseeker_id = s.id INNER JOIN bonus b ON i.bonus_id = b.id;